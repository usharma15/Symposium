...
The live-stream registry is the capacity and privacy-safe telemetry authority. It enforces twelve streams per client and 500 per process while recording only aggregate active/peak connections, resume attempts, capacity rejection, replay counts, replay failure/truncation, backlog, and write-pressure signals. The scheduled production watch reads those aggregates from database-silent readiness every two hours, verifies a valid Render release identity, checks public web security/availability, and retains evidence without requesting bootstrap or waking Neon. A release-time manual dispatch accepts an expected backend SHA and then enforces exact identity; the scheduled path cannot assume the current default-branch SHA because docs-only and frontend-only commits intentionally do not rebuild Render.
...
- Reshare and quote notifications retain distinct aggregation keys, labels, analytics destinations, and durable preference controls. Migration `0048_quote_notification_preference` preserves every existing user's prior combined choice by copying `reshares` into the new `quotes` preference before separating future delivery.
