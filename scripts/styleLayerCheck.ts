...
const layers = [
  ...
  "89-communities.css",
  "89-community-announcements.css",
  "89-community-people.css",
...
const main = async () => {
  ...
  assert.match(scribbleStyles, /\.scribble-panel\s*\{[^}]*var\(--symposium-side-tool-top\)[^}]*var\(--symposium-side-tool-bottom\)/);
  assert.match(tabletStyles, /\.tablet-panel\s*\{[^}]*width:\s*var\(--symposium-side-tool-width\)/);
  assert.match(tabletStyles, /\.tablet-panel\s*\{[^}]*var\(--symposium-side-tool-top\)[^}]*var\(--symposium-side-tool-bottom\)/);
