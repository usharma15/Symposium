...
assert.match(systemRoutes, /probeDatabase: request\.query\.probe === "database"/);
assert.match(apiSmoke, /\/readyz\?probe=database/);
assert.match(renderBlueprint, /healthCheckPath: \/healthz/);
assert.match(renderBlueprint, /buildFilter:[\s\S]*paths:[\s\S]*- apps\/api\/\*\*/);
assert.match(renderBlueprint, /DATABASE_APPLICATION_NAME[\s\S]*value: symposium-api-render/);
