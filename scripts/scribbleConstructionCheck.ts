...
assert.match(source.context, /symposiumRecoveryRetryDelayMs\(retryAttempt - 1\)/);
assert.match(source.context, /useSymposiumRecoveryRefresh/);
assert.match(source.context, /window\.addEventListener\("pagehide"/);
assert.doesNotMatch(source.context, /window\.addEventListener\("online"/);
assert.match(source.context, /browserRecoveryCoordinator\.subscribe/);
