...
const main = async () => {
  ...
  assert.equal(verificationManifest.length, 62);
  assert.equal(new Set(verificationManifest.map((stage) => stage.id)).size, 62);
  ...
  assert.equal(selectVerificationStages(["assistant"], []).length, 9);
