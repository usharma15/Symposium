...
import { createLocalWorkspaceNotebook } from "@/lib/localWorkspaceStore";
...
export async function POST(request: Request) {
  ...
  const live = await proxyLiveBackend("/v1/workspace/notebooks", {
    method: "POST",
    body: payload,
    actorHandle,
  ...
  if (live) return live;
  try {
    return privateWorkspaceResponse(await createLocalWorkspaceNotebook(payload, actorHandle));
  } catch (error) {
